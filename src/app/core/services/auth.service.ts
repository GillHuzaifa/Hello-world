...
export class AuthService {
  ...
  login(
    ...
    const endpoint = portal
      ...
      : `${this.API}/auth/login`;
    try {
      // debug: log endpoint & payload to help diagnose 401 vs Postman
      // eslint-disable-next-line no-console
      console.debug('[AuthService] login endpoint:', endpoint, 'body:', body);
    } catch { }

    return this.http.post<TokenResponse>(endpoint, body).pipe(
      tap(response => {
        this.storeToken(response.access_token);
        // Store a basic user object with identifier to ensure user data exists
        // Full user data will be fetched on subsequent API calls
        const basicUser: AuthUser = { email: identifier };
        try {
          localStorage.setItem('pulseq_user', JSON.stringify(basicUser));
        } catch { }
      }),
      switchMap(() =>
        this.fetchCurrentUser().pipe(
          map(() => true),
          catchError(() => {
            // Even if fetchCurrentUser fails, we already have basic user data
            console.warn('[AuthService] Failed to fetch full user data, but basic user info is stored');
            return of(true);
          })
        )
      ),
  ...
  pharmacyLogin(email: string, password: string): Observable<boolean> {
    ...
    try {
      // debug: log endpoint & payload for pharmacy login
      // eslint-disable-next-line no-console
      console.debug('[AuthService] pharmacyLogin endpoint:', `${this.API}/auth/pharmacy/login`, 'body:', body);
    } catch { }

    return this.http.post<TokenResponse>(`${this.API}/auth/pharmacy/login`, body).pipe(
      tap(response => {
        this.storeToken(response.access_token);
        // Store a basic user object with pharmacy email to ensure user data exists
        // Full user data will be fetched on subsequent API calls
        const basicUser: AuthUser = { email };
        try {
          localStorage.setItem('pulseq_user', JSON.stringify(basicUser));
        } catch { }
      }),
      switchMap(() =>
        this.fetchCurrentUser().pipe(
          ...
          catchError(() => {
            // Even if fetchCurrentUser fails, we already have basic user data
            // so the pharmacy portal can function
            console.warn('[AuthService] Failed to fetch full user data, but basic user info is stored');
            return of(true);
          })
        )
  ...
  restoreSession(): void {
    try {
      ...
      console.warn('[AuthService] Failed to restore session, clearing invalid data:', error);
      // ⚠️ Do NOT call logout() here — logout() calls router.navigate() which
      // is NOT safe during APP_INITIALIZER (router is not yet ready).
      // Just clear the bad data directly from localStorage.
      try {
        localStorage.removeItem('pulseq_token');
        localStorage.removeItem('pulseq_user');
      } catch { }
