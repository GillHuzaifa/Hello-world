...
export class AdminManageDoctorsComponent implements OnInit, OnDestroy {
    ...
    loadDoctorsFromApi(): void {
        ...
        this.doctorService.manageDoctors({ hospitalId, pageSize: 100 }).subscribe({
            next: (res: any) => {
                ...
                if (Array.isArray(docs)) {
                    this.doctors = docs.map((d: any) => ({
                        id: d.id || d._id || '',
                        name: d.name || '',
                        specialization: d.department || d.subcategory || d.specialization || '',
                        department: d.department || d.subcategory || '',
                        qualifications: d.qualifications || '',
                        timings: `${this.convertTo12Hour(d.start_time || d.startTime || '')} – ${this.convertTo12Hour(d.end_time || d.endTime || '')}`,
                        available: d.status === 'available',
                        fee: d.consultation_fee ? `Rs. ${d.consultation_fee}` : (d.fee ? `Rs. ${d.fee}` : ''),
                        consultation_fee: d.consultation_fee || d.fee || 0,
                        onLeave: d.status === 'on_leave' || d.status === 'on leave' || d.status === 'onLeave',
                        phone: d.phone || '',
                        email: d.email || '',
                        rating: d.rating || 0,
    ...
    loadDepartments(): void {
        this.doctorService.listAdminDepartments().subscribe({
            next: (res: any) => {
    ...
    saveAdd(): void {
        ...
        if (!this.addModel.phone?.trim() || this.addModel.phone.trim().length < 10) {
            this.messageService.add({
                severity: 'error', summary: 'Validation Error',
                detail: 'Phone number is required and must be at least 10 digits', life: 3000
            });
            return;
        }
        ...
        this.doctorService.createDoctor(payload as any).subscribe({
            ...
            error: (err) => {
                ...
                if (err?.error?.detail && Array.isArray(err.error.detail)) {
                    ...
                    detail = `Request failed with status ${err.status}. Please verify API availability and payload.`;
                }
                this.messageService.add({
