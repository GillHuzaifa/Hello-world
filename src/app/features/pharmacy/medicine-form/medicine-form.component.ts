...
@Component({
    ...
    imports: [
        ...
        AutoCompleteModule, RippleModule,
...
export class MedicineFormComponent implements OnInit {
    ...
    distributorNameOptions: string[] = [];
    distributorCompanyOptions: string[] = [];

    filteredSuppliers: string[] = [];
    filteredDistributorNames: string[] = [];
    filteredDistributorCompanies: string[] = [];
    ...
    ngOnInit(): void {
        this.initializeForm();
        this.loadSupplierAndDistributorOptions();
        this.checkIfEditing();
    ...
    loadSupplierAndDistributorOptions(): void {
        this.pharmacyService.fetchAllMedicines().subscribe({
            next: (medicines: any[]) => {
                const suppliers = new Set<string>();
                const distributorNames = new Set<string>();
                const distributorCompanies = new Set<string>();

                medicines.forEach((med: any) => {
                    if (med.distributor) distributorNames.add(med.distributor);
                    if (med.distributor) distributorCompanies.add(med.distributor);
                });

                this.supplierOptions = Array.from(suppliers).sort();
                this.distributorNameOptions = Array.from(distributorNames).sort();
                this.distributorCompanyOptions = Array.from(distributorCompanies).sort();
                this.filteredSuppliers = this.supplierOptions;
                this.filteredDistributorNames = this.distributorNameOptions;
                this.filteredDistributorCompanies = this.distributorCompanyOptions;
            },
            error: (err) => {
                console.error('Failed to load supplier/distributor options', err);
                this.supplierOptions = [];
                this.distributorNameOptions = [];
                this.distributorCompanyOptions = [];
            }
        });
    }
    ...
    filterSuppliers(event: any): void {
        ...
        if (!this.filteredSuppliers.includes(event.query) && event.query) {
            this.filteredSuppliers.unshift(event.query);
        }
    }

    filterDistributorNames(event: any): void {
        const query = (event.query || '').toLowerCase();
        this.filteredDistributorNames = this.distributorNameOptions.filter(d =>
            d.toLowerCase().includes(query)
        );
        if (!this.filteredDistributorNames.includes(event.query) && event.query) {
            this.filteredDistributorNames.unshift(event.query);
        }
    }

    filterDistributorCompanies(event: any): void {
        const query = (event.query || '').toLowerCase();
        this.filteredDistributorCompanies = this.distributorCompanyOptions.filter(d =>
            d.toLowerCase().includes(query)
        );
        if (!this.filteredDistributorCompanies.includes(event.query) && event.query) {
            this.filteredDistributorCompanies.unshift(event.query);
        }
    }
            ...
            this.pharmacyService.fetchAllMedicines().subscribe({
                next: (medicines: any[]) => {
                    ...
                    if (medicine) {
                        ...
                        const expiryDate = this.parseDateForCalendar(medicine.expiration_date);
    ...
    private parseDateForCalendar(value: any): Date | null {
        ...
        if (dmySlash) {
            return new Date(+dmySlash[3], +dmySlash[2] - 1, +dmySlash[1]);
        }

        const dmyDash = text.match(/^(\d{1,2})-(\d{1,2})-(\d{4})$/);
        if (dmyDash) {
            return new Date(+dmyDash[3], +dmyDash[2] - 1, +dmyDash[1]);
        }
