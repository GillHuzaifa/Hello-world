...
import { Invoice, InvoiceCounts, InvoiceItem, InvoiceListParams } from '../../../shared/models/invoice.model';
...
export class InvoicesComponent implements OnInit {
    ...
    loadInvoices(params?: InvoiceListParams): void {
        ...
        this.invoiceService.getAllInvoices(params).subscribe({
            next: (res) => {
                const rawInvoices = (res as any)?.data?.invoices || (Array.isArray((res as any)?.data) ? (res as any).data : []);
                let filtered = (rawInvoices || []).filter((invoice: Invoice) => !invoice?.is_deleted);

                // If search term exists, filter by invoice number, customer, OR medicine names
                if (params?.search && this.searchTerm.trim()) {
                    const searchLower = this.searchTerm.toLowerCase();
                    filtered = filtered.filter((invoice: Invoice) =>
                        invoice.invoice_number?.toLowerCase().includes(searchLower) ||
                        invoice.customer_name?.toLowerCase().includes(searchLower) ||
                        (invoice.items || []).some((item: InvoiceItem) => item.product_name?.toLowerCase().includes(searchLower))
                    );
                }

                this.invoices = filtered;

                const apiCounts = (res as any)?.data?.counts;
                if (apiCounts) {
                    this.counts = apiCounts;
                } else {
                    this.counts = {
                        all: this.invoices.length,
                        completed: this.invoices.filter(i => i.status === 'completed').length,
                        pending: this.invoices.filter(i => i.status === 'pending').length,
                        partial: this.invoices.filter(i => i.status === 'partial').length,
                        cancelled: this.invoices.filter(i => i.status === 'cancel').length
                    };
                }
                this.loading = false;
                this.cdr.markForCheck();
    ...
    getTotalUniqueCustomers(): number {
        const names = this.invoices
            .map(inv => inv.customer_name?.trim().toLowerCase())
            .filter(Boolean);
        return new Set(names).size;
    }

    getTotalItems(): number {
        return this.invoices.reduce((sum, inv) => sum + (inv.item_count || 0), 0);
    }
    ...
    printInvoice(invoice: Invoice): void {
        ...
        this.invoiceService.getInvoice(invoice.id).subscribe({
            next: (res) => {
                ...
                this.cdr.markForCheck();
                // Use a longer timeout to ensure Angular renders all content
                setTimeout(() => {
    ...
    viewInvoice(invoice: Invoice): void {
        if (!invoice.id) return;
        this.router.navigate([pharmacyPath('invoices/create')], {
    ...
    goToTrash(): void {
        this.router.navigate([pharmacyPath('invoices/trash')]);
    }
