...
export class ReceptionDashboardComponent implements OnInit, OnDestroy {
  ...
  private processQueueResponse(res: any): void {
    ...
    this.allTokens = tokensArray.map((t: any) => {
      ...
      else if (statusRaw === 'WAITING' || statusRaw === 'waiting' || statusRaw === 'pending') mappedStatus = 'pending';
    ...
    // ── FIX: Remove stale skipped tokens that are actually completed on backend ──
    if (this.skippedTokens.length > 0) {
      const completedTokenNumbers = this.allTokens
        .filter(t => t.status === 'completed')
        .map(t => t.token);
      const staleSkipped = this.skippedTokens.filter(t => completedTokenNumbers.includes(t.token));
      if (staleSkipped.length > 0) {
        staleSkipped.forEach(t => this.removeFromSkippedStorage(t.token));
      }
    }
  ...
  loadDashboardStats(): void {
    ...
    this.staffService.getReceptionistDashboard(hid).pipe(takeUntil(this.destroy$)).subscribe({
      next: (res: any) => {
        ...
        if (data.upcoming_queue && Array.isArray(data.upcoming_queue) && data.upcoming_queue.length > 0) {
  ...
  reAddToQueue(patient: Patient): void {
    ...
    this.receptionService.reAddToken(patient.id)
      ...
      .subscribe({
        ...
        // ── FIX: Handle completed token re-add error gracefully ──
        error: (err) => {
          ...
          if (msg.toLowerCase().includes('completed')) {
            // Token is already completed on the backend — remove from stale skipped list
            this.removeFromSkippedStorage(patient.token);
            const tokenInAll = this.allTokens.find(t => t.token === patient.token);
            if (tokenInAll) {
              tokenInAll.status = 'completed';
            }
            this.syncSkippedFromAllTokens();
            this.refreshUpcomingList();
            this.messageService.add({
              severity: 'info', summary: 'Already Completed',
              detail: `${patient.token} - ${patient.name} was already completed and has been removed from skipped list`,
              life: 4000
            });
          } else {
            this.messageService.add({
              severity: 'error', summary: 'Error',
              detail: err?.error?.message || 'Failed to re-add patient to queue', life: 3000
            });
          }
        }
      });
